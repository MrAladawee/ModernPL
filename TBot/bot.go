...
// Структуры для распарсивания ответа от API поиска сотрудника
...
// Структуры для распарсивания ответа расписания
...
// Структура для хранения данных пользователя (в памяти; для production используйте БД)
...
func getEmployee(fullName string) (Employee, error) {
	// Формируем URL запроса, подставляя ФИО (если необходимо, экранируем пробелы)
...
// Функция для получения расписания по teacherID с использованием нового URL
func getSchedule(teacherID int) ([]Subject, error) {
	// Формируем URL запроса расписания согласно новому шаблону:
	// https://auth.kpfu.tyuop.ru/api/v1/employees/ID_ПОЛЬЗОВАТЕЛЯ/schedule
